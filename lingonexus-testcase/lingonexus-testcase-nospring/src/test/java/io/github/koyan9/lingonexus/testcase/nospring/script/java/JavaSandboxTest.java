...
package io.github.koyan9.lingonexus.testcase.nospring.script.java;
...
import io.github.koyan9.lingonexus.script.java.JavaSandbox;
...
/**
 * Tests for JavaSandbox (Janino)
 */
class JavaSandboxTest {
    ...
    void setUp() {
        ...
        sandbox = new JavaSandbox(config);
    }

    @Test
    @DisplayName("Should support Java language identifiers")
    void shouldSupportJava() {
        Assertions.assertTrue(sandbox.supports("java"));
        Assertions.assertTrue(sandbox.supports("janino"));
        Assertions.assertFalse(sandbox.supports("javaexpr")); // "javaexpr" refers to liquor-eval
    ...
    void shouldReturnCorrectLanguage() {
        ...
        assertEquals("java", compiled.getLanguage());
    }
}
